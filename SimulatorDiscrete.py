...
from ProsumerGUROBI_FIX import Prosumer, Manager, apply_beta_gamma_weights
...
class Simulator(Simulation):
    def __init__(self, config=None):
        ...
        self.config["simulator"] = self
        for i in range(self.nag):
            for j in self.partners[i]:
                self.trust_scores[i][j] = 1.0
...
class PlayerOptimizationMsg(Event):
    ...
    def process(self, sim: Simulator):
        ...
        mx = 10 + random.randint(0, 2) if sim.isLatency else 10
