...
# 1) Load only TRUST runs, skip any convergences before iteration 5
df = pd.read_csv('local_conv_trust.csv')
df = df[df['iter'] >= 5]
...
for tval in t_vals:
    ...
    for x in xticks:
        ...
        for i in range(len(y)-1):
            if abs(y[i+1] - y[i]) > 3:
                if i-start+1 > 1: clusters.append(list(range(start,i+1)))
                start = i+1
        if len(y)-start > 1: clusters.append(list(range(start,len(y))))
    ...
    # only show multiples of 100
    max_iter = int(df_t['iter'].max() + 50)
    ticks = list(range(100, max_iter+1, 100))
    ax.set_yticks(ticks)
