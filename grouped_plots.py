...
constants, no run-ids).  For Residuals groups it also produces two separate
plots: one overlaying the two runs with identical parameters, and one
overlaying a reference run plus the third, distinct-parameter run.
...
def plot_series(dfs: List[pd.DataFrame], ycol: str, labels: List[str],
                title: str, out_p: Path):
    fig, ax = plt.subplots(figsize=(7, 5))
    for df, lbl in zip(dfs, labels):
        ax.plot(df["iter_shift"], df[ycol], label=lbl)
    ax.set_xscale("log"); ax.set_yscale("log")
    ax.set_xlabel("Iteration"); ax.set_ylabel(ycol)
    ax.set_title(title); ax.legend(ncol=1)
    plt.tight_layout(); fig.savefig(out_p); plt.close(fig)

def plot_residuals(dfs: List[pd.DataFrame], labels: List[str],
                   title: str, out_p: Path):
    fig, ax = plt.subplots(figsize=(7, 5))
    for df, lbl in zip(dfs, labels):
        ax.plot(df["iter_shift"], df["prim"], label=f"prim • {lbl}")
        ax.plot(df["iter_shift"], df["dual"],
                linestyle="--", label=f"dual • {lbl}")
    ax.set_xscale("log"); ax.set_yscale("log")
...
def param_signature(tag: str) -> Tuple[str, str, str]:
    d = extract_params(tag)
    return (d.get("t",""), d.get("p",""), d.get("α",""))

def make_labels(tags: List[str]) -> List[str]:
    params_per_tag = [extract_params(t) for t in tags]
    varying = {k for k in ("t","p","α")
               if len({d.get(k) for d in params_per_tag}) > 1}
    labels = []
    for d in params_per_tag:
        parts = [f"{k}={d[k]}" for k in ("t","p","α") if k in varying and k in d]
...
def main():
    ap = argparse.ArgumentParser(
        description="Overlay plots with variable-only legends")
    ...
    for idx, grp in enumerate(GROUPS, 1):
        ...
        for t in tags:
            try:
                dfs.append(load_run(t)); ok_tags.append(t)
            except FileNotFoundError:
                print(f"  ⚠  Missing CSV for {t}; skipping.")

        if metric != "Residuals":
            if not dfs:
                print("  ↳  no data, skipped.")
                continue
            labels = make_labels(ok_tags)
            out_pdf = OUT_DIR / f"{metric}_grp{idx}.pdf"
            PLOT_FNS[metric](dfs, labels, title, out_pdf)
            print(f"  ↳  saved {out_pdf}")
            if args.show: plt.show(block=False)
            continue

        # ────────── Special handling for Residuals ──────────
        if len(ok_tags) < 2:
            print("  ↳  insufficient data for Residuals, skipped.")
            continue

        sig_map: Dict[Tuple[str,str,str], List[str]] = {}
        for t in ok_tags:
            sig_map.setdefault(param_signature(t), []).append(t)

        dup_sigs = [sig for sig,lst in sig_map.items() if len(lst)==2]
        if len(dup_sigs) != 1:
            labels = make_labels(ok_tags)
            out_pdf = OUT_DIR / f"{metric}_grp{idx}.pdf"
            plot_residuals(dfs, labels, title, out_pdf)
            print(f"  ↳  saved fallback {out_pdf}")
            if args.show: plt.show(block=False)
            continue

        dup_sig  = dup_sigs[0]
        dup_tags = sig_map[dup_sig]
        uniq_sig = next(sig for sig in sig_map if sig != dup_sig)
        uniq_tag = sig_map[uniq_sig][0]

        # A) duplicates-only
        dfs_dup = [load_run(t) for t in dup_tags]
        labels_dup = ["run 1", "run 2"]
        pval       = extract_params(dup_tags[0])["p"]
        out_dup    = OUT_DIR / f"{metric}_grp{idx}_dupes.pdf"
        plot_residuals(dfs_dup, labels_dup,
                       f"{title} (p={pval})", out_dup)
        print(f"  ↳  saved {out_dup}")

        # B) reference+distinct
        dfs_mix    = [load_run(dup_tags[0]), load_run(uniq_tag)]
        labels_mix = [
            "run 1",
            f"p={extract_params(uniq_tag)['p']}"
        ]
        out_mix    = OUT_DIR / f"{metric}_grp{idx}_mix.pdf"
        plot_residuals(dfs_mix, labels_mix, title, out_mix)
